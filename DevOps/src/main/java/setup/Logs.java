...
public class Logs {
	...
	public static void collectLogs(long experimentStart, String expName) throws InterruptedException, IOException {
		...
		String deployment = Util.sendCommandWithReturn("10.1.3.32", "kubectl get pod -o=custom-columns=NAME:.metadata.name,STATUS:.status.phase,NODE:.spec.nodeName -n default");
		...
		String token = Util.sendCommandWithReturn("10.1.3.32", "gcloud auth application-default print-access-token ").trim();
		String utilLogs = Util.sendCommandWithReturn("10.1.3.32", "curl -X GET -H \"Authorization: Bearer " + token + "\" \"https://monitoring.googleapis.com/v3/projects/devops-218113/timeSeries?filter=metric.type%3D%22compute.googleapis.com%2Finstance%2Fcpu%2Futilization%22&interval.endTime=" + endTime + "&interval.startTime=" + startTime + "\"");
